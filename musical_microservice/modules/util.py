...
def convert_tempo_model_to_pb(tempo : musical_parameters.Tempo) -> pb.Tempo:
    return pb.Tempo(
        beats_per_min=tempo.beats_per_min,
        marking=tempo.marking
    )

def convert_time_signature_model_to_pb(time_signature : musical_parameters.TimeSignature) -> pb.TimeSignature:
    return pb.TimeSignature(
        beats_per_measure=time_signature.beats_per_measure,
        beat_value=time_signature.beat_value
    )

def convert_musical_parameters_model_to_pb(parameters : musical_parameters.ParameterAnnotation) -> pb.MusicalParameters:
